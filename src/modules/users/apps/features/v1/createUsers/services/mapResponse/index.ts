...
export interface ICreateUserMapResponse extends IServiceHandlerAsync<UserEntity,CreateUserResponseDto>{}

@sealed
@Service()
export class CreateUserMapResponseService implements ICreateUserMapResponse {
  public async handleAsync(params: UserEntity): Promise<Result<CreateUserResponseDto, ResultError>> {
    try
    {
      //@guard
      if(!params)
