...
class DeactivateUserCommandHandler
	implements RequestHandler<DeactivateUserCommand, ApiDataResponse<AesResponseDto>>
{
	...
	public constructor() {
		...
		this._deactivateUserRequestValidationService = Container.get(
			DeactivateUserRequestValidationService
		);
		this._activeDeactivateUsersDbService = Container.get(ActiveDeactivateUsersDbService);
		this._deactivatedUserEncryptResponseService = Container.get(
	...
	public async handle(value: DeactivateUserCommand): Promise<ApiDataResponse<AesResponseDto>> {
		...
		try {
			...
			// Validation Service
			const deactivateValidationServiceResult =
				await this._deactivateUserRequestValidationService.handleAsync({
					dto: request,
			...
			if (deactivateValidationServiceResult.isErr()) {
				logger.error(
					logConstruct(
						...
						`DeactivateUserRequestValidationService`,
						deactivateValidationServiceResult.error.message
					)
				);
				return DataResponseFactory.error(
					deactivateValidationServiceResult.error.status,
					deactivateValidationServiceResult.error.message
				);
			}
			...
			const activeDeactivateUsersDbServiceResult =
				await this._activeDeactivateUsersDbService.handleAsync({
			...
			// Map Response
			const deactivateUserResponseDto: DeactivateUserResponseDto =
				new DeactivateUserResponseDto();
			deactivateUserResponseDto.message = `User Deactivated Successfully`;

			// Encrypt Response
			const encryptResponseServiceResult =
				await this._deactivatedUserEncryptResponseService.handleAsync({
			...
			logger.info(
				logConstruct(`DeactivateUserCommandHandler`, `handle`, `Release Transaction`)
			);
		}
	}
}
