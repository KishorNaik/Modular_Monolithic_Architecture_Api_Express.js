...
export class GetUsersController {
	...
	public async get(
		@QueryParams() request: GetUsersRequestDto,
		@Req() req: Request,
		@Res() res: Response
	) {
...
export class GetUserQueryHandler
	implements RequestHandler<GetUsersQuery, ApiDataResponse<AesResponseDto>>
{
	...
	public async handle(value: GetUsersQuery): Promise<ApiDataResponse<AesResponseDto>> {
		try {
			...
			const page: PaginationDataResponseModel =
				itemPagesResult.page as PaginationDataResponseModel;

			const getUsersResponseDto: Array<GetUsersResponseDto> = items;

			logger.info(
				logConstruct(`GetUserQueryHandler`, `GetUsersDbService`, `Get Users Success`)
			);
			...
			// Encrypt
			const getUsersDecryptResponseServiceResult =
				await this._getUsersDecryptResponseService.handleAsync({
			...
			const aesResponseDto: AesResponseDto =
				getUsersDecryptResponseServiceResult.value.aesResponseDto;
			logger.info(
				logConstruct(
					`GetUserQueryHandler`,
					`GetUsersDecryptResponseService`,
