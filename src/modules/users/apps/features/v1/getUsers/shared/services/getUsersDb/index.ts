...
import { GetUsersService, IGetUsersServiceParameters, IPageListResult, UserEntity } from "@kishornaik/mma_db";
...
export interface IGetUserDbService extends IServiceHandlerAsync<IGetUserDbServiceParameters,IPageListResult<IUsersForAdmin>>{}
...
export class GetUsersDbService implements IGetUserDbService{
  ...
  public constructor(){
    ...
    this._getUsersFromEntityToTypeMapperService=Container.get(GetUsersFromEntityToTypeMapperService);
  }

  public async handleAsync(params: IGetUserDbServiceParameters): Promise<Result<IPageListResult<IUsersForAdmin>, ResultError>> {
    try
    {
      //@guard
      if(!params)
      ...
      if(getUserServiceResult.isErr())
        return ResultExceptionFactory.error(getUserServiceResult.error.statusCode,getUserServiceResult.error.message);
      ...
      // Map Service
      const getUsersFromEntityToTypeMapperServiceResult=await this._getUsersFromEntityToTypeMapperService.handleAsync(users);
      if(getUsersFromEntityToTypeMapperServiceResult.isErr())
        return ResultExceptionFactory.error(getUsersFromEntityToTypeMapperServiceResult.error.status,getUsersFromEntityToTypeMapperServiceResult.error.message);
