...
class GetUserByIdentifierQueryHandler
	implements RequestHandler<GetUserByIdentifierQuery, ApiDataResponse<AesResponseDto>>
{
	...
	public constructor() {
		...
		this._getUserByIdentifierValidationRequestService = Container.get(
			GetUserByIdentifierValidationRequestService
		);
		this._getUsersByIdentifierResponseMapper = Container.get(
			GetUsersByIdentifierResponseMapperService
		);
		this._getUsersByIdentifierResponseEncryptService = Container.get(
	...
	public async handle(value: GetUserByIdentifierQuery): Promise<ApiDataResponse<AesResponseDto>> {
      ...
      if(!value.expressRequest)
        return DataResponseFactory.error(StatusCodes.BAD_REQUEST, 'Invalid express request')
			...
			if (!userId) {
				logger.error(
					logConstruct(`GetUserByIdentifierQueryHandler`, `handle`, `Unauthorized`)
				);
			...
			logger.info(logConstruct(`GetUserByIdentifierQueryHandler`, `handle`, `Authorized`));

			// Validation Service
			const getUserByIdentifierValidationRequestServiceResult =
				await this._getUserByIdentifierValidationRequestService.handleAsync({
					dto: request,
			...
			if (getUserByIdentifierValidationRequestServiceResult.isErr()) {
				...
				return DataResponseFactory.error(
					getUserByIdentifierValidationRequestServiceResult.error.status,
					getUserByIdentifierValidationRequestServiceResult.error.message
				);
			}
			...
			// Map Response
			const getUsersByIdentifierResponseMapperServiceResult =
				await this._getUsersByIdentifierResponseMapper.handleAsync(users);
			if (getUsersByIdentifierResponseMapperServiceResult.isErr()) {
				...
				return DataResponseFactory.error(
					getUsersByIdentifierResponseMapperServiceResult.error.status,
					getUsersByIdentifierResponseMapperServiceResult.error.message
				);
			}
			const responseDto: GetUserByIdentifierResponseDto =
				getUsersByIdentifierResponseMapperServiceResult.value;
			logger.info(
				logConstruct(
					`GetUserByIdentifierQueryHandler`,
					`getUsersByIdentifierResponseMapper`,
			...
			// Encrypt Response
			const getUsersByIdentifierResponseEncryptServiceResult =
				await this._getUsersByIdentifierResponseEncryptService.handleAsync({
					data: responseDto,
			...
			if (getUsersByIdentifierResponseEncryptServiceResult.isErr()) {
				...
				return DataResponseFactory.error(
					getUsersByIdentifierResponseEncryptServiceResult.error.status,
					getUsersByIdentifierResponseEncryptServiceResult.error.message
				);
			}
			logger.info(
			...
			const response: AesResponseDto =
				getUsersByIdentifierResponseEncryptServiceResult.value.aesResponseDto;
			// Return Response
			return DataResponseFactory.success(StatusCodes.OK, response);
		} catch (ex) {
			const error = ex as Error;
			logger.error(
				logConstruct(`GetUserByIdentifierQueryHandler`, `handle`, error.message, ex)
			);
