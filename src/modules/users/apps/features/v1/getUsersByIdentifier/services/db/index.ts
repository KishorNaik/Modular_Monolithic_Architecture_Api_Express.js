...
Container.set<GetUsersByIdentifierService>(GetUsersByIdentifierService,new GetUsersByIdentifierService());

export interface IGetUsersByIdentifierDbService extends IServiceHandlerAsync<UserEntity, UserEntity> {}
  ...
  public async handleAsync(params: UserEntity): Promise<Result<UserEntity, ResultError>> {
    try
    {
      ...
      if(getUserByIdentifierServiceResult.isErr())
        return ResultExceptionFactory.error(getUserByIdentifierServiceResult.error.statusCode,getUserByIdentifierServiceResult.error.message);
