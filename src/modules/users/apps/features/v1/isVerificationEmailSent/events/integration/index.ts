...
import { IIIsVerificationEmailSendUpdateDbService, IsVerificationEmailSendUpdateDbService } from "./services/db";
...
export class IsVerificationEmailSendIntegrationEventServiceHandler implements NotificationHandler<IsVerificationEmailSendIntegrationEventService> {
  ...
  public constructor(){
    this._isVerificationEmailSendIntegrationEventValidationService=Container.get(IsVerificationEmailSendIntegrationEventValidationService);
    this._isVerificationEmailSendUpdateDbService=Container.get(IsVerificationEmailSendUpdateDbService);
    this._userSharedCacheService=Container.get(UserSharedCacheService);
  }

  public async handle(notification: IsVerificationEmailSendIntegrationEventService): Promise<void> {
    ...
    try{
      ...
      // Validation
      const isVerificationEmailSendIntegrationEventValidationServiceResult=await this._isVerificationEmailSendIntegrationEventValidationService.handleAsync({
        dto:request,
      ...
      if(isVerificationEmailSendIntegrationEventValidationServiceResult.isErr())
        throw new Error(isVerificationEmailSendIntegrationEventValidationServiceResult.error.message);

      // Db Service
      await queryRunner.startTransaction();

      const isVerificationEmailSendUpdateDbServiceResult=await this._isVerificationEmailSendUpdateDbService.handleAsync({
        request:request,
