...
export interface IsJwtAndRefreshTokenService extends IServiceHandlerVoidAsync<IsValidJwtAndRefreshTokenServiceParameters>{
}
...
export class IsValidJwtAndRefreshTokenService implements IsJwtAndRefreshTokenService {
  ...
  public async handleAsync(params: IsValidJwtAndRefreshTokenServiceParameters): Promise<Result<undefined, ResultError>> {
    try
    {
      //@Guard
      if(!params)
      ...
      if(!isRefreshTokenMatched)
        return ResultExceptionFactory.error(StatusCodes.UNAUTHORIZED,`unauthorized access`);

      // Check if user id is matched or not
      const getUserIdResult=await this._userTokenProviderService.getUserIdByJwtToken(accessToken);
      const isUserIdMatched=getUserIdResult===params.users.identifier;
      if(!isUserIdMatched)
        return ResultExceptionFactory.error(StatusCodes.UNAUTHORIZED,`unauthorized access`);
