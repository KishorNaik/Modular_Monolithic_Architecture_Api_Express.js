...
import { getQueryRunner, initializeDatabase, StatusEnum, UserEntity, UserKeysEntity } from '@kishornaik/mma_db';
...
import { IUserSignInGenerateJwtAndRefreshTokenServiceResult, UserSignInGenerateJwtAndRefreshTokenService } from './services/generateJwtAndRefresh';
...
class UserSignInCommandHandler implements RequestHandler<UserSignInCommand, ApiDataResponse<AesResponseDto>> {
  ...
  public constructor(){
    ...
    this._userSignRequestValidationService=Container.get(UserSignInRequestDtoValidationService);
    this._getUserByEmailIdService=Container.get(GetUsersByEmailIdService);
    this._userSignInValidateCredentialsService=Container.get(UserSignInValidateCredentialsService);
  ...
  public async handle(value: UserSignInCommand): Promise<ApiDataResponse<AesResponseDto>> {
    ...
    try
    {
      ...
      const userSignInDecryptRequestServiceResult=await this._userSignInDecryptRequestService.handleAsync({
        data:request.body,
        key:ENCRYPTION_KEY
      });
      if(userSignInDecryptRequestServiceResult.isErr())
      {
        ...
        return DataResponseFactory.error(userSignInDecryptRequestServiceResult.error.status,userSignInDecryptRequestServiceResult.error.message);
      }

      const userSignInRequestDto:UserSignInRequestDto=userSignInDecryptRequestServiceResult.value;
      logger.info(
				logConstruct(
					`UserSignInCommandHandler`,
						`UserSignInDecryptRequestService`,
      ...
      // Validation Service
      const userSignInValidationServiceResult=await this._userSignRequestValidationService.handleAsync({
      ...
      if(userSignInValidationServiceResult.isErr()){
        ...
        return DataResponseFactory.error(userSignInValidationServiceResult.error.status,userSignInValidationServiceResult.error.message);
      }
      logger.info(
				logConstruct(
          `UserSignInCommandHandler`,
      ...
      if(getUserByEmailIdServiceResult.isErr()){
        ...
        if(error.status===StatusCodes.NOT_FOUND)
          return DataResponseFactory.error(StatusCodes.UNAUTHORIZED,`username and password is incorrect`);
      ...
      const userSignInValidateCredentialsServiceResult=await this._userSignInValidateCredentialsService.handleAsync({
        users:users,
      ...
      if(userSignInValidateCredentialsServiceResult.isErr()){
        ...
        return DataResponseFactory.error(userSignInValidateCredentialsServiceResult.error.status,userSignInValidateCredentialsServiceResult.error.message);
      }
      logger.info(
				logConstruct(
          `UserSignInCommandHandler`,
      ...
      // Map Entity Service
      const mapUserSignInEntityServiceResult=await this._mapUserSignInEntityService.handleAsync({
        users:users,
      ...
      if(mapUserSignInEntityServiceResult.isErr())
      {
        ...
        return DataResponseFactory.error(mapUserSignInEntityServiceResult.error.status,mapUserSignInEntityServiceResult.error.message);
      }
      ...
      const updateUserRefreshTokenServiceResult=await this._userSignInUpdateDbService.handleAsync({
        entity:{
      ...
      if(updateUserRefreshTokenServiceResult.isErr()){
        ...
        return DataResponseFactory.error(updateUserRefreshTokenServiceResult.error.status,updateUserRefreshTokenServiceResult.error.message);
      }
      ...
      const mapUserSignInResponseServiceResult=await this._mapUserSignInResponseService.handleAsync({
        token:{
      ...
      if(mapUserSignInResponseServiceResult.isErr()){
        ...
        return DataResponseFactory.error(mapUserSignInResponseServiceResult.error.status,mapUserSignInResponseServiceResult.error.message);
      }

      const userSignInResponseDto:UserSignInResponseDto=mapUserSignInResponseServiceResult.value;

      //Encrypt Response
      const userSignInEncryptRequestServiceResult=await this._userSignInEncryptResponseService.handleAsync({
      ...
      if(userSignInEncryptRequestServiceResult.isErr()){
        ...
        return DataResponseFactory.error(userSignInEncryptRequestServiceResult.error.status,userSignInEncryptRequestServiceResult.error.message);
      }

      const aesResponseDto:AesResponseDto=userSignInEncryptRequestServiceResult.value.aesResponseDto;
