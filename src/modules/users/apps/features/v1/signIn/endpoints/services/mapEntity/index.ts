...
export interface IMapUserSignInEntityService extends IServiceHandlerAsync<IMapUserSignInEntityServiceParameters,IMapUserSignInEntityServiceResults>{}

@sealed
@Service()
export class MapUserSignInEntityService implements IMapUserSignInEntityService{
  public async handleAsync(params: IMapUserSignInEntityServiceParameters): Promise<Result<IMapUserSignInEntityServiceResults, ResultError>> {
    try
    {
      //@Guard
      if(!params)
