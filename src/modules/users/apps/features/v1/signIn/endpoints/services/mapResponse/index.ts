...
export interface IMapUserSignInResponseService extends IServiceHandlerAsync<IMapUserSignInResponseServiceParameters,UserSignInResponseDto>{}

@sealed
@Service()
export class MapUserSignInResponseService implements IMapUserSignInResponseService {
  public async handleAsync(params: IMapUserSignInResponseServiceParameters): Promise<Result<UserSignInResponseDto, ResultError>> {
    try
    {
      // @Guard
      if(!params)
