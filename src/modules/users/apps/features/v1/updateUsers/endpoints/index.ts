...
import {
	IMapUpdateUserRequestToEntityResults,
	MapUpdateUserRequestToEntityService,
} from './services/entityMap';
...
export class UpdateUserController {
	...
	@UseBefore(
		...
		ValidationMiddleware(AesRequestDto)
	)
...
class UpdateUserCommandHandler
	implements RequestHandler<UpdateUserCommand, ApiDataResponse<AesResponseDto>>
{
	...
	public constructor() {
		...
		this._updateUserValidationRequestService = Container.get(
			UpdateUserValidationRequestService
		);
		this._mapUpdateUserRequestToEntityService = Container.get(
			MapUpdateUserRequestToEntityService
		);
	...
	public async handle(value: UpdateUserCommand): Promise<ApiDataResponse<AesResponseDto>> {
		...
		try {
			...
			// Decrypt Service
			const updateUserDecryptRequestServiceResult =
				await this._updateUserDecryptRequestService.handleAsync({
					data: request.body,
					key: aesToken,
				});
			if (updateUserDecryptRequestServiceResult.isErr()) {
				logger.error(
					logConstruct(
						`UpdateUserCommandHandler`,
						`UpdateUserDecryptRequestService`,
						updateUserDecryptRequestServiceResult.error.message
					)
				);
				return DataResponseFactory.error(
					updateUserDecryptRequestServiceResult.error.status,
					updateUserDecryptRequestServiceResult.error.message
				);
			}

			const updateUserRequestDto: UpdateUserRequestDto =
				updateUserDecryptRequestServiceResult.value;
			logger.info(
				logConstruct(
					`UpdateUserCommandHandler`,
					`UpdateUserDecryptRequestService`,
			...
			if (!userId) {
				logger.error(logConstruct(`UpdateUserCommandHandler`, `handle`, `Unauthorized`));
			...
			if (users.status === StatusEnum.INACTIVE) {
				logger.error(logConstruct(`UpdateUserCommandHandler`, `handle`, `Forbidden`));
			...
			// Validation Service
			const updateUserValidationRequestServiceResult =
				await this._updateUserValidationRequestService.handleAsync({
			...
			if (updateUserValidationRequestServiceResult.isErr()) {
				logger.error(
					logConstruct(
						`UpdateUserCommandHandler`,
						`UpdateUserValidationRequestService`,
						updateUserValidationRequestServiceResult.error.message
					)
				);
				return DataResponseFactory.error(
					updateUserValidationRequestServiceResult.error.status,
					updateUserValidationRequestServiceResult.error.message
				);
			}
			logger.info(
			...
			// Entity Map Service
			const mapUpdateUserRequestToEntityServiceResult =
				await this._mapUpdateUserRequestToEntityService.handleAsync({
			...
			if (mapUpdateUserRequestToEntityServiceResult.isErr()) {
				logger.error(
					logConstruct(
						`UpdateUserCommandHandler`,
						`MapUpdateUserRequestToEntityService`,
						mapUpdateUserRequestToEntityServiceResult.error.message
					)
				);
				return DataResponseFactory.error(
					mapUpdateUserRequestToEntityServiceResult.error.status,
					mapUpdateUserRequestToEntityServiceResult.error.message
				);
			}

			const entity: IMapUpdateUserRequestToEntityResults =
				mapUpdateUserRequestToEntityServiceResult.value;
			logger.info(
				logConstruct(
					`UpdateUserCommandHandler`,
					`MapUpdateUserRequestToEntityService`,
			...
			if (updateUserDbServiceResult.isErr()) {
				...
				return DataResponseFactory.error(
					updateUserDbServiceResult.error.status,
					updateUserDbServiceResult.error.message
				);
			}
			logger.info(
			...
			// Encrypt Response
			const updateUserEncryptResponseServiceResult =
				await this._updateUserEncryptResponseService.handleAsync({
			...
			if (updateUserEncryptResponseServiceResult.isErr()) {
				logger.error(
					logConstruct(
						`UpdateUserCommandHandler`,
						`UpdateUserEncryptResponseService`,
						updateUserEncryptResponseServiceResult.error.message
					)
				);
				await queryRunner.rollbackTransaction();
				logger.error(
				...
				return DataResponseFactory.error(
					updateUserEncryptResponseServiceResult.error.status,
					updateUserEncryptResponseServiceResult.error.message
				);
			}
			const aesResponseDto: AesResponseDto =
				updateUserEncryptResponseServiceResult.value.aesResponseDto;
			logger.info(
				logConstruct(
					`UpdateUserCommandHandler`,
					`UpdateUserEncryptResponseService`,
