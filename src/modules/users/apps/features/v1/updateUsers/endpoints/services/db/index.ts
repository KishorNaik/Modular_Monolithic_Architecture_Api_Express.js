...
export interface IUpdateUsersDbService
	extends IServiceHandlerVoidAsync<IUpdateUsersDbServiceParameters> {}
...
export class UpdateUserDbService implements IUpdateUsersDbService {
	...
	public async handleAsync(
		params: IUpdateUsersDbServiceParameters
		...
		try {
			...
			const updateUserCommunicationServiceResult =
				await this._updateUserCommunicationService.handleAsync(communication, queryRunner);
			if (updateUserCommunicationServiceResult.isErr())
				return ResultExceptionFactory.error(
					updateUserCommunicationServiceResult.error.statusCode,
					updateUserCommunicationServiceResult.error.message
				);

			// Update User Credentials
			const updateUserCredentialsServiceResult =
				await this._updateUserCredentialsService.handleAsync(credentials, queryRunner);
			if (updateUserCredentialsServiceResult.isErr())
				return ResultExceptionFactory.error(
					updateUserCredentialsServiceResult.error.statusCode,
					updateUserCredentialsServiceResult.error.message
				);
