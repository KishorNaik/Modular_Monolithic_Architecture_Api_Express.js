...
import { IServiceHandlerAsync, IServiceHandlerVoid, IServiceHandlerVoidAsync } from "@/shared/utils/helpers/services";
import { QueryRunner, StatusEnum, UpdateUserSettingsService, UserEntity, UserSettingsEntity } from "@kishornaik/mma_db";
...
export interface IIfTokenIsNotValidService extends IServiceHandlerAsync<IIfTokenIsNotValidServiceParameters,IIfTokenIsNotValidServiceResult>{}
...
export class IfTokenIsNotValidService implements IIfTokenIsNotValidService{
  ...
  public async handleAsync(params: IIfTokenIsNotValidServiceParameters): Promise<Result<IIfTokenIsNotValidServiceResult, ResultError>> {
    try
    {
      // @guard
      if(!params)
      ...
      const userSettingsEntity:UserSettingsEntity=mapIUsersToEntityServiceResult.value.userSettingsEntity;
      ...
      if(updateEmailTokenServiceResult.isErr())
        return ResultExceptionFactory.error(updateEmailTokenServiceResult.error.status,updateEmailTokenServiceResult.error.message);

      //Update Row Version
      const updateUserRowVersionServiceResult=await this._updateUserRowVersionService.handleAsync({
        userId:userEntity.identifier,
        status:StatusEnum.INACTIVE,
        queryRunner: queryRunner
      });
      if(updateUserRowVersionServiceResult.isErr())
        return ResultExceptionFactory.error(updateUserRowVersionServiceResult.error.status,updateUserRowVersionServiceResult.error.message);

      // Update Cache result
      const updateUserSharedCacheServiceResult=await this._userSharedCacheService.handleAsync({
        identifier:userId,
        queryRunner:queryRunner,
        status:StatusEnum.INACTIVE
      });
      if(updateUserSharedCacheServiceResult.isErr())
        return ResultExceptionFactory.error(updateUserSharedCacheServiceResult.error.status,updateUserSharedCacheServiceResult.error.message);
...
interface IMapIUsersToEntityService extends IServiceHandlerAsync<IMapIUsersToEntityServiceParameters,IMapIUsersToEntityServiceResult>{
}

@sealed
@Service()
class MapIUsersToEntityService implements IMapIUsersToEntityService{
  public async handleAsync(params: IMapIUsersToEntityServiceParameters): Promise<Result<IMapIUsersToEntityServiceResult, ResultError>> {
    try
    {
      // @guard
      if(!params)
      ...
      userSettingsEntity.email_Verification_Token_expires_at=new Date(new Date().getTime() + 24 * 60 * 60 * 1000);
...
interface IUpdateEmailTokenService extends IServiceHandlerVoidAsync<IUpdateEmailTokenServiceParameters>{
}
...
class UpdateEmailTokenService implements IUpdateEmailTokenService{
  ...
  public async handleAsync(params: IUpdateEmailTokenServiceParameters): Promise<Result<undefined, ResultError>> {
    try{
      if(!params)
        return ResultExceptionFactory.error(StatusCodes.BAD_REQUEST,"Invalid params");

      if(!params.userSettingEntity)
        return ResultExceptionFactory.error(StatusCodes.BAD_REQUEST,"Invalid userSettingEntity");
      ...
      const updateUserSettingEntityResult=await this._updateUserSettingEntity.handleAsync(userSettingEntity,queryRunner);
      if(updateUserSettingEntityResult.isErr())
        return ResultExceptionFactory.error(updateUserSettingEntityResult.error.statusCode,updateUserSettingEntityResult.error.message);
