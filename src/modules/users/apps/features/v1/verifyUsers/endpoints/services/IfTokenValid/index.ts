...
export interface IIfTokenValidService
	extends IServiceHandlerAsync<IIfTokenValidServiceParameters, IIfTokenValidServiceResult> {}
...
export class IfTokenValidService implements IIfTokenValidService {
	...
	public async handleAsync(
		params: IIfTokenValidServiceParameters
	): Promise<Result<IIfTokenValidServiceResult, ResultError>> {
		try {
			// @Gaurd
			if (!params)
			...
			// Map Entity
			const mapIUsersToEntityServiceResult =
				await this._mapIUsersToEntityService.handleAsync(users);
			...
			// Update Status
			const updateUserDbServiceResult = await this._updateUsersDbService.handleAsync(
				entity.entity.user,
				queryRunner
			);
			...
			const updateUserCommunicationDbServiceResult =
				await this._updateUserCommunicationDbService.handleAsync(
					entity.entity.communication,
					queryRunner
				);
			if (updateUserCommunicationDbServiceResult.isErr())
				return ResultExceptionFactory.error(
					updateUserCommunicationDbServiceResult.error.statusCode,
					updateUserCommunicationDbServiceResult.error.message
				);

			const updateUserCredentialsDbServiceResult =
				await this._updateUserCredentialsDbService.handleAsync(
					entity.entity.credentials,
					queryRunner
				);
			if (updateUserCredentialsDbServiceResult.isErr())
				return ResultExceptionFactory.error(
					updateUserCredentialsDbServiceResult.error.statusCode,
					updateUserCredentialsDbServiceResult.error.message
				);

			const updateUserSettingsDbServiceResult =
				await this._updateUserSettingsDbService.handleAsync(
					entity.entity.settings,
					queryRunner
				);
			if (updateUserSettingsDbServiceResult.isErr())
				return ResultExceptionFactory.error(
					updateUserSettingsDbServiceResult.error.statusCode,
					updateUserSettingsDbServiceResult.error.message
				);

			const updateUserKeysDbServiceResult = await this._updateUserKeyDbService.handleAsync(
				entity.entity.keys,
				queryRunner
			);
			if (updateUserKeysDbServiceResult.isErr())
				return ResultExceptionFactory.error(
					updateUserKeysDbServiceResult.error.statusCode,
					updateUserKeysDbServiceResult.error.message
				);

			// Cache Update
			const userSharedCacheUpdateServiceResult =
			...
			if (userSharedCacheUpdateServiceResult.isErr())
				return ResultExceptionFactory.error(
					userSharedCacheUpdateServiceResult.error.status,
					userSharedCacheUpdateServiceResult.error.message
				);
...
interface IMapIUsersToEntityService
	extends IServiceHandlerAsync<IUsers, IMapIUsersToEntityServiceResult> {}
...
class MapIUsersToEntityService implements IMapIUsersToEntityService {
	public async handleAsync(
		...
		try {
			...
			const userCommunicationEntityEntity: UserCommunicationEntity =
