...
export interface IIsEmailTokenValidService
	extends IServiceHandlerAsync<UserSettingsEntity, GetEmailVerificationTokenServiceResult> {}
...
export class IsEmailTokenValidService implements IIsEmailTokenValidService {
	...
	public async handleAsync(
		params: UserSettingsEntity
	): Promise<Result<GetEmailVerificationTokenServiceResult, ResultError>> {
		...
		try {
			...
			const getEmailVerificationTokenServiceResult =
				await this._getEmailVerificationTokenService.handleAsync({
			...
			if (getEmailVerificationTokenServiceResult.isErr()) {
				...
				if (error.includes('entity not found'))
					return ResultExceptionFactory.error(
						...
						`Email Verification Token is not valid`
					);

				return ResultExceptionFactory.error(
					getEmailVerificationTokenServiceResult.error.statusCode,
					getEmailVerificationTokenServiceResult.error.message
				);
			}

			const userSettingEntity: UserSettingsEntity =
				getEmailVerificationTokenServiceResult.value;

			// Check Email verification token with expire data
			const emailTokenExpiredAt =
				userSettingEntity.email_Verification_Token_expires_at.toString();
