...
export interface IGetUsersByEmailIdService extends IServiceHandlerAsync<IGetUsersByEmailIdServiceParameters, IUsers>{
}
...
export class GetUsersByEmailIdService implements IGetUsersByEmailIdService{
  ...
  public async handleAsync(params: IGetUsersByEmailIdServiceParameters): Promise<Result<IUsers, ResultError>> {
    try
    {
      // @Guard
      if(!params)
      ...
      if(getUserByEmailIdCacheServiceResult.isErr())
        return ResultExceptionFactory.error(getUserByEmailIdCacheServiceResult.error.status,getUserByEmailIdCacheServiceResult.error.message);

      const cacheValue: string = getUserByEmailIdCacheServiceResult.value;

      // If User Not Found
      if(!cacheValue)
        return ResultExceptionFactory.error(StatusCodes.NOT_FOUND,`User with emailId ${params.emailId} not found`);
